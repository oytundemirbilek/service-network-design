...
# class ServiceNetworkModelWithHubs(OptimizationModel):
#     """Model that solves a service coverage problem by finding the most optimal hubs."""
...
#     ) -> None:
#         super().__init__()
#         self.model = gp.Model("Service Network with Hubs")
#         self.n_nodes = n_nodes
#         self.n_hubs = n_hubs
#         self.price_per_km = price_per_km
#         self.base_price = base_price
...
#         demands_redirected = np.copy(demands)
#         for i, demand_loc in enumerate(demands):
#             for j, demand in enumerate(demand_loc):
#                 is_src_hub = hub_indices[i]
#                 is_dest_hub = hub_indices[j]
#                 if is_src_hub and is_dest_hub:
#                     continue
#                 src_hub = np.where(hub_zones[i] == 1)
#                 dest_hub = np.where(hub_zones[j] == 1)
#                 if len(src_hub) == 0 or len(dest_hub) == 0:
#                     continue
#                 if is_src_hub and not is_dest_hub:
#                     if src_hub != dest_hub:
#                         # demands_redirected[i, src_hub] += demand
#                         demands_redirected[src_hub, dest_hub] += demand
#                         demands_redirected[dest_hub, j] += demand
#                         demands_redirected[i, j] = 0

#                 elif not is_src_hub and is_dest_hub:
#                     if src_hub != dest_hub:
#                         demands_redirected[i, src_hub] += demand
#                         demands_redirected[src_hub, dest_hub] += demand
#                         # demands_redirected[dest_hub, j] += demand
...
#                 elif not is_src_hub and not is_dest_hub:
#                     if src_hub != dest_hub:
#                         demands_redirected[i, src_hub] += demand
#                         demands_redirected[src_hub, dest_hub] += demand
#                         demands_redirected[dest_hub, j] += demand
